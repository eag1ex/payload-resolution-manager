...
- Easy to use full featured _Data Management Service_ for async handling with Node.js
- Good for:
...
- You can Provide async/deferred data, and chain the calls, it will wait... Then continue after data becomes available, this is possible with the help of `XPromise/XPipe` (from another tool) available at : `https://github.com/eag1ex/xpromise` - take a look for more details.
...
- **Nice demo examples in** `./examples/screens/..` **example:** `node ./examples/screens/screen-4.js`
...
var d = prm
  ...
  .compute((items) => {
    var allNewItems = items.map((zz, inx) => {
      return {
        name: zz.dataSet.name,
        surname: "anonymous",
        age: zz.dataSet.age + inx + 1,
      };
    });
...
/**
 * if you prefer to return each resolution separately:
      ...
      prm
        ...
        .compute((items) => {
          return items.map((zz, inx) => {
            ...
            return Object.assign({}, orVals, {
              status: "updated",
              age: orVals.age + 10 + inx,
            });
          });
        }, "all")
        .resolution();
...
- `batchReady(jobUIDS=[], type:string, cb=>)`: You want to wait until specific jobs has completed. Each job in batch is set uppon resolution is called, each time it checks if all your batch jobs are ready.
  ...
  - `jobUIDS` :specify working job uids - `type`: can return as `flat`> array, or `grouped`> object - `cb:` when ready returns callback
...
- `compute(callback(), method='all',uid)`: use this method to perform data calculation for each `job:uid`.
  ...
  - `uid`: provide for data if not chaining, or switching to another job. When `uid`=null it will look for last used. If anonymous, because your data was async, must provide `formatted()` > with {dataSet[],\_uid,\_ri} so it can search thru and match available.
...
- `pipe(cb=>, uid)` : its an extension `from XPromise/Xpipe` more information at : `https://github.com/eag1ex/xpromise`. To use this feature with PRM you have to set `opts.asAsync=true`, examples available at `./examples/app.async.example`
